...
function formatPhoneNumber(inputStr) {
  ...
  let areaCode = null;
  if (returnStr) {
    areaCode = returnStr.countryCallingCode
      ? returnStr.countryCallingCode
      : null;
...
function constructJson(contactInfo, chatUnreadCount) {
  ...
  tempJSON.unreadMessage = chatUnreadCount[contactInfo.id]
    ? chatUnreadCount[contactInfo.id]
    : 0;
      ...
      contactInfo.contactData.map((profileDetails, i) => {
        if (profileDetails.name === 'contactNo') {
          ...
          if (
            ...
            profileDetails.value.length > 1
          ) {
            let otherName = null;
            tempJSON.otherName = profileDetails.value.forEach(function(
              item,
              index
            ) {
              if (index !== 0) {
  ...
  switch (action.type) {
    ...
    case FILTER_USERS: {
      if (action.payload === "") {
        return {
          ...state,
          chatUsers: state.contactList
        };
      } else {
        return {
          ...state,
          chatUsers: state.chatUsers.filter(
            createFilter(action.payload, ["name", "emailId", "contactNo"])
          )
        };
      }
    }
    ...
    case ON_SELECT_USER: {
      return {
        ...
        drawerState: false,
        selectedSectionId:
          action.payload && action.payload.user ? action.payload.user.id : "",
        selectedUser: "",
    ...
    case UPDATE_LIVE_SUPPORT: {
      return {
        ...state,
        updatedLiveSupportRequestList: action.payload
      };
    }
    ...
    case FETCH_ALL_CHAT_USER_SUCCESS: {
      ...
      if (userData.contacts.length > 0) {
        newUserData = userData.contacts
          .map(element => constructJson(element, userData.unreadCount))
          .sort(function(a, b) {
            return b.recentActivityDate < a.recentActivityDate
              ? -1
              : b.recentActivityDate > a.recentActivityDate
              ? 1
              : 0;
          });
      }
